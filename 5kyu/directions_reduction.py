...
Going to one direction and coming back the opposite direction right away is a needless effort. Since this is the wild west, with
dreadful weather and not much water, it's important to save yourself some energy, otherwise you might die of thirst!
...
You can immediately see that going "NORTH" and immediately "SOUTH" is not reasonable, better stay to the same place! So the task is to
...
def directions_reduction(arr):
    ...
    new_arr = []
    for side in arr:
        if len(new_arr) == 0 or new_arr[-1] != opposite[side]:
            new_arr.append(side)
        else:
            new_arr.pop()
...
print(directions_reduction(["NORTH", "EAST", "WEST", "SOUTH", "WEST", "WEST"]))