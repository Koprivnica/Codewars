...
Calculate the tram's minimum capacity such that the number of people inside the tram never exceeds this capacity
at any time. Note that at each stop all exiting passengers exit before any entering passenger enters the tram.
...
def tram(stops, descending, onboarding):
    ...
    for i in range(0, stops):
        current_num = current_num - descending[i] + onboarding[i]
        min_capacity = max(min_capacity, current_num)
          
    return min_capacity

print(tram(4, [0, 2, 4, 4], [3, 5, 2, 0]))